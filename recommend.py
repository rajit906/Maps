...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    return group_by_first([[find_closest(restaurant_location(ree), centroids),ree] for ree in restaurants ])
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        list_1.append(restaurant_location(restaurant)[0])
        list_2.append(restaurant_location(restaurant)[1])
    return [mean(list_1),mean(list_2)]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    "*** YOUR CODE HERE ***"
    meanx = mean(list_of_x_values)
    meany = mean(list_of_y_values)
    sxx = sum([pow(x - meanx, 2) for x in list_of_x_values])
    syy = sum([pow(y - meany, 2) for y in list_of_y_values])
    sxy = sum([(x-meanx)*(y-meany) for x,y in zip(list_of_x_values,list_of_y_values)])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for name in restaurants.keys():
        if name in user_reviewed.keys():
            ...
            d[name] = predictor(ALL_RESTAURANTS[name])
